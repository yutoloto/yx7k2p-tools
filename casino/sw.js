...
/* 同じオリジンに麻雀アプリと旅しおりが同居している。消すのは自分の古いキャッシュ（casino-）だけ */
self.addEventListener("activate", e => {
  e.waitUntil(caches.keys()
    .then(ks => Promise.all(ks.filter(k => k.startsWith("casino-") && k !== CACHE).map(k => caches.delete(k))))
    .then(() => self.clients.claim()));
});

/* 画面はネットワーク優先（更新をすぐ反映）／落ちたらキャッシュ＝電車の圏外でも開く */
